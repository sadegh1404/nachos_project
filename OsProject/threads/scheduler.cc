...
Scheduler::ReadyToRun (Thread *thread)
{
    ...
    switch(policy)
    {
    	...
    	case SCHED_MLQ: 
    	case SCHED_PRIO_NP: // Nonpreemptive Priority scheduling
    	...
    	case SCHED_PRIO_P:
    	case SCHED_RR:
    	case SCHED_FCFS: // FCFS algorithm
        readyList->Append(thread);
			  break;
			
      case SCHED_SJF: // SJF scheduling algorithm
        readyList->SortedInsert(thread,thread->getCBT());
        break;
...
Scheduler::ShouldISwitch ( Thread  * oldThread, Thread * newThread )
{
   ...
   switch(policy)
   {
      ...
      if(oldThread->getPriority() < newThread->getPriority())
   	...
   	case SCHED_FCFS: // FCSF scheduling algorithm
      doSwitch = false;
		  break;
